...
app.secret_key = os.environ.get("SECRET_KEY", b"_53oi3uriq9pifpff;apl")
...
def signup():
    ...
    if not all([email, first_name, last_name, password, confirm_password]):
        return render_template(
            "login.html", signup_error="All fields are required.", user=None
        )
    if password != confirm_password:
        return render_template(
            "login.html", signup_error="Passwords do not match.", user=None
        )

    # Check if user already exists
    if dbHandler.check_credentials(email, password):
        return render_template(
            "login.html", signup_error="User already exists.", user=None
        )
...
def samplesyllabus():
    user = session.get("user")
    return render_template("samplesyllabus.html", user=user)
